window.addEventListener("scroll", () => {
  ...
  document.querySelectorAll("section").forEach((section, currentSection) => {
    ...
    if (section.offsetTop <= scrollDistance) {
      ...
      document
        ...
        [currentSection].classList.add("navigation__item--active");
      ...
      document
        ...
        [currentSection].classList.add("menu__item--active");
    }
  });
  ...
  if (window.innerHeight + window.pageYOffset >= document.body.offsetHeight) {
    ...
    document
      .getElementById("last_child-menu")
      .classList.add("menu__item--active");
  }

  const DISTANCE = 80;
  // distance - дистанция между последним и предпоследним элементом
  if (
    window.innerHeight + window.pageYOffset + DISTANCE >=
